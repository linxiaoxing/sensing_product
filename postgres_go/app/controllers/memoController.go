...
func sendNotificationMessage() {
    ...
    if err != nil {
        log.Fatalf("Firebase Admin SDK初期化エラー: %v", err)
    }
    ...
    message := &messaging.Message{
        ...
        Token: "fXoyfuOpSNWo-P_VCPSwqU:APA91bFDT0f4rI6DIVLvErIjst55-XG14VHUF8AxwW90U0AUF8Q0CUMqI4UqCq9MJmRLH4TlXIcgGf5YIOQ7qdLDVycuwltzbAgBDueh5sjGzQbL0DOcGZ0OGIUof6SAJUknTNPNAbmP", // 送信先のデバイストークンを指定
    }

    // メッセージを送信
    client, err := app.Messaging(context.Background())
    if err != nil {
        log.Fatalf("Messagingクライアント取得エラー: %v", err)
    }

    response, err := client.Send(context.Background(), message)
    if err != nil {
        log.Fatalf("プッシュ通知送信エラー: %v", err)
    }

    // レスポンスのログ出力
    log.Printf("プッシュ通知送信成功: %v", response)
}